...
useEffect(() => {
  ...
  return (
    <div className="body-part">
      <h1 className="body-part__title">
        Exercise Library
      </h1>
      <h2 className="body-part__title-sub">Choose a body part to view associated exercises</h2>

      <div className="body-part-container">
        {bodyPartData.map((bodyPart) => (
          <Link to={`/body/${bodyPart.id}/exercises`}
          className="body-part-card" key={bodyPart.id}>
            <h2>{bodyPart.name}</h2>
          </Link>
        ))}
      </div>
